...
#pragma unroll
    ...
    if (s_merged != nullptr) {
        s_merged[pos * num_heads + head_idx] =
            gpu_iface::math::ptx_log2(s_a_val + s_b_val) + s_max;
    }
}
...
#pragma unroll
    ...
    if (s != nullptr) {
        s[pos * num_heads + head_idx] =
            gpu_iface::math::ptx_log2(s_val + s_other_val) + s_max;
    }
}
...
#pragma unroll
    for (uint32_t iter = 0; iter < num_smem_stages; ++iter) {
        gpu_iface::memory::pred_load<vec_bits, PrefetchMode::kPrefetch,
...
gpuError_t MergeState(DTypeIn *v_a,
                      float *s_a,
                      DTypeIn *v_b,
                      float *s_b,
                      DTypeO *v_merged,
...
gpuError_t MergeStateInPlace(DType *v,
                             float *s,
                             DType *v_other,
                             float *s_other,
                             uint32_t seq_len,
 ...
 */
template <typename DTypeIn, typename DTypeO>
gpuError_t MergeStates(DTypeIn *v,
                       float *s,
                       DTypeO *v_merged,
                       float *s_merged,
                       uint32_t num_index_sets,
...
template <typename DTypeIn, typename DTypeO>
gpuError_t AttentionSum(DTypeIn *v,
                        DTypeO *v_sum,
                        uint32_t num_index_sets,
                        uint32_t seq_len,
                        uint32_t num_heads,
...
gpuError_t VariableLengthMergeStates(DTypeIn *v,
                                     float *s,
                                     IdType *indptr,
                                     DTypeO *v_merged,
                                     float *s_merged,
...
template <typename DTypeIn, typename DTypeO, typename IdType>
gpuError_t VariableLengthAttentionSum(DTypeIn *v,
                                      IdType *indptr,
                                      DTypeO *v_sum,
                                      uint32_t max_seq_len,
                                      uint32_t *seq_len,
