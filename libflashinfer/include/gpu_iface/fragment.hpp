...
struct fragment_t {
  ...
  // for such use cases.
  static constexpr int elements_per_thread = (frag_type == FragmentType::accumulator) ? 8
                                             : (sizeof(T) == 1)                       ? 8
                                                                                      : 4;
  ...
  __device__ __forceinline__ T& operator[](int i) { return reinterpret_cast<T*>(data)[i]; }
  __device__ __forceinline__ const T& operator[](int i) const {
    return reinterpret_cast<const T*>(data)[i];
  }

  // Get number of elements this thread holds
  __device__ __forceinline__ constexpr int size() const { return elements_per_thread; }
  ...
  using rocwmma_layout =
      typename std::conditional<frag_type == FragmentType::row_major, rocwmma::row_major,
                                typename std::conditional<frag_type == FragmentType::col_major,
                                                          rocwmma::col_major, void>::type>::type;

  using rocwmma_matrix_t = typename std::conditional<
      frag_type == FragmentType::row_major, rocwmma::matrix_a,
      typename std::conditional<frag_type == FragmentType::col_major, rocwmma::matrix_b,
  ...
  using rocwmma_frag_t = typename std::conditional<
      frag_type == FragmentType::accumulator, rocwmma::fragment<rocwmma_matrix_t, M, N, K, T>,
  ...
  __device__ __forceinline__ const rocwmma_frag_t* raw_ptr() const { return &frag; }
#endif
...
template <typename T>
using row_major_fragment_m16n16k16 = fragment_t<T, 16, 16, 16, FragmentType::row_major>;

template <typename T>
using col_major_fragment_m16n16k16 = fragment_t<T, 16, 16, 16, FragmentType::col_major>;

template <typename T>
using accumulator_fragment_m16n16k16 = fragment_t<T, 16, 16, 16, FragmentType::accumulator>;
