...
#include "utils_hip.h"
  ...
  std::vector<dtype_out> single_mha(
      ...
      PosEncodingMode pos_encoding_mode = PosEncodingMode::kNone, float logits_soft_cap = 8.0f,
      float rope_scale = 1.f, float rope_theta = 1e4, bool use_soft_cap = false) {
      ...
      for (size_t qo_head_idx = 0; qo_head_idx < num_qo_heads; ++qo_head_idx) {
        ...
        for (size_t q_idx = 0; q_idx < qo_len; ++q_idx) {
          ...
          for (size_t feat_idx = 0; feat_idx < head_dim; ++feat_idx) {
            ...
            for (size_t kv_idx = 0; kv_idx < kv_len; ++kv_idx) {
              o_float +=
                  att[kv_idx] * fi::con::explicit_casting<dtype_kv, float>(
  ...
  void append_paged_kv_cache(
      paged_kv_t<T, IdxType> page_cpu, const std::vector<std::vector<T>>& keys,
      const std::vector<std::vector<T>>& values, const std::vector<IdxType>& append_indptr) {
