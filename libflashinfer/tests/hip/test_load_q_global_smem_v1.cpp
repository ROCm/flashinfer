...
constexpr uint32_t QUERY_ELEMS_PER_THREAD = 4;  // Each thread loads 4 fp16 elements
...
void SimulateOffsetPattern(std::vector<int>& thread_ids_at_offsets) {
  ...
  constexpr uint32_t COLUMN_RESET_OFFSET = (NUM_MMA_D_QK / 4) * WARP_STEP_SIZE;
  constexpr uint32_t grid_width = (HEAD_DIM / QUERY_ELEMS_PER_THREAD);  // 16 for 64, 32 for 128
  constexpr uint32_t grid_height = 16 * NUM_MMA_Q;  // 16 for NUM_MMA_Q=1, 32 for NUM_MMA_Q=2
  ...
  for (uint32_t tid = 0; tid < 64; tid++) {
    ...
    uint32_t q_smem_offset_w = get_permuted_offset_linear<UPCAST_STRIDE_Q>(row, col);
    ...
    for (uint32_t mma_q = 0; mma_q < NUM_MMA_Q; ++mma_q) {
      for (uint32_t j = 0; j < 4; ++j) {
        ...
        for (uint32_t mma_do = 0; mma_do < NUM_MMA_D_QK / 4; ++mma_do) {
          ...
          if (q_smem_offset_w < grid_height * grid_width) {  // Safety check
          ...
          q_smem_offset_w =
              advance_offset_by_column_linear<WARP_STEP_SIZE>(q_smem_offset_w, mma_do);
        }

        // Advance to next sequence (row) with adjustment back to first
        // column
        q_smem_offset_w =
            advance_offset_by_row_linear<WARP_THREAD_ROWS, UPCAST_STRIDE_Q>(q_smem_offset_w) -
            COLUMN_RESET_OFFSET;
      }
    }
  }
...
void RunOffsetTest() {
  ...
  for (int c = 0; c < grid_width; c++) {
    printf("%3d ", c);
    if (c == 15 && grid_width > 16) printf("| ");  // Divider between first and second half
  }
  printf("\n   +");
  for (int c = 0; c < grid_width; c++) {
    printf("----");
  ...
  for (int r = 0; r < grid_height; r++) {
    ...
    if (r == 15 && NUM_MMA_Q > 1) {
      ...
      for (int c = 0; c < grid_width; c++) {
        printf("----");
        if (c == 15 && grid_width > 16) printf("+");  // Intersection divider
      }
      printf("\n");
    }
  }
