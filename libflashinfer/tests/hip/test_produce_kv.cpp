...
void SimulateProduceKV(std::vector<int>& thread_ids_at_offsets) {
  ...
  constexpr uint32_t ELEMS_PER_THREAD = 4;  // Each thread loads 4 fp16 elements
  ...
  for (uint32_t tid = 0; tid < WARP_SIZE; tid++) {
    ...
    uint32_t kv_smem_offset_w =
        get_permuted_offset_linear<UPCAST_STRIDE>(warp_idx * WARP_THREAD_ROWS + row, col);
    ...
    for (uint32_t i = 0; i < NUM_MMA_KV * 4 / NUM_WARPS_Q; ++i) {
      ...
      for (uint32_t j = 0; j < NUM_MMA_D / 4; ++j) {
        ...
        // }
        if (kv_smem_offset_w < grid_height * grid_width && kv_idx < grid_height) {
          thread_ids_at_offsets[kv_smem_offset_w] = tid;
        } else {
          std::cerr << "ERROR: Out of bound offset (" << kv_smem_offset_w << ") at " << tid << '\n';
        }

        // Advance to next column by 16 (number of threads per row)
        kv_smem_offset_w = advance_offset_by_column_linear<WARP_STEP_SIZE>(kv_smem_offset_w, j);
        // if(tid == 0) {
      ...
      kv_smem_offset_w = advance_offset_by_row_linear<NUM_WARPS * WARP_THREAD_ROWS, UPCAST_STRIDE>(
                             kv_smem_offset_w) -
                         COLUMN_RESET_OFFSET;
...
template <uint32_t HEAD_DIM, uint32_t NUM_MMA_KV = 1>
void RunProduceKVTest() {
  constexpr uint32_t grid_width = HEAD_DIM / 4;      // 16 for 64, 32 for 128
  constexpr uint32_t grid_height = 16 * NUM_MMA_KV;  // 16 for NUM_MMA_KV=1, 32 for NUM_MMA_KV=2
  ...
  // Print statistics
  printf("\nStatistics:\n");
  printf("- Positions written: %d/%d (%.1f%%)\n", grid_height * grid_width - unwritten,
         grid_height * grid_width,
         100.0f * (grid_height * grid_width - unwritten) / (grid_height * grid_width));
  printf("- Unwritten positions: %d/%d (%.1f%%)\n", unwritten, grid_height * grid_width,
         100.0f * unwritten / (grid_height * grid_width));
}

TEST(KVCacheWritePatternTest, HeadDim64_AMD_kLinear) { RunProduceKVTest<64, 1>(); }

TEST(KVCacheWritePatternTest, HeadDim128_AMD_kLinear) { RunProduceKVTest<128, 1>(); }
