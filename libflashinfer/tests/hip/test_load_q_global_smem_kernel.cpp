        ...
        if (q_idx < qo_len) {
            for (size_t feat_idx = 0; feat_idx < head_dim; ++feat_idx) {
                ...
                size_t global_offset =
                    q_idx * q_stride_n + r * q_stride_h + feat_idx;
...
uint_fastdiv create_group_size_div(uint32_t group_size)
{
    return uint_fastdiv(group_size);
}
    ...
    uint_fastdiv group_size_div)
{
    ...
    load_q_global_smem<KTraits>(qo_packed_idx_base, qo_len, q_global,
                                q_stride_n, q_stride_h, group_size_div, &q_smem,
                                threadIdx);
...
// Main test function
template <typename DTypeQ> bool test_q_loading_correctness()
{
    std::cout << "Testing Q loading correctness with " << sizeof(DTypeQ) * 8
    ...
    FI_GPU_CALL(hipMemcpy(q_device, q_host.data(), q_size * sizeof(DTypeQ),
                          hipMemcpyHostToDevice));

    // Define kernel traits for CDNA3
    using KTraits =
        KernelTraits<MaskMode::kNone, 16, 1, 1, 4, 4, 1, 1,
                     PosEncodingMode::kNone, DTypeQ, DTypeQ, DTypeQ, float,
    ...
    std::cout << "  Block size: " << block_size.x << "x" << block_size.y << "x"
              << block_size.z << std::endl;
    std::cout << "  Shared memory: " << shared_mem_size << " bytes"
              << std::endl;
    ...
    test_q_loading_kernel<KTraits><<<grid_size, block_size, shared_mem_size>>>(
        q_device, q_smem_output, qo_packed_idx_base, qo_len, q_stride_n,
    ...
    FI_GPU_CALL(hipMemcpy(q_smem_actual.data(), q_smem_output,
                          smem_elements * sizeof(DTypeQ),
                          hipMemcpyDeviceToHost));

    // Generate CPU reference
    std::vector<DTypeQ> q_smem_expected = cpu_reference_q_smem_layout(
        q_host, qo_len, num_qo_heads, head_dim, q_stride_n, q_stride_h,
    ...
    std::cout << "Comparing " << q_smem_actual.size() << " elements..."
              << std::endl;

    for (size_t i = 0;
         i < std::min(q_smem_actual.size(), q_smem_expected.size()); ++i)
    {
        float actual =
            fi::con::explicit_casting<DTypeQ, float>(q_smem_actual[i]);
        float expected =
            fi::con::explicit_casting<DTypeQ, float>(q_smem_expected[i]);
    ...
    std::cout << "Total mismatches: " << mismatch_count << " / "
              << q_smem_actual.size() << std::endl;
    std::cout << "Q loading test: " << (passed ? "PASSED" : "FAILED")
              << std::endl;
...
int main()
{
    ...
    std::cout << "Testing load_q_global_smem function for CDNA3 architecture"
              << std::endl;

    // Initialize HIP
    hipError_t err = hipSetDevice(0);
    if (err != hipSuccess) {
        std::cout << "Failed to set HIP device: " << hipGetErrorString(err)
                  << std::endl;
        return 1;
    }
