...
template <typename T>
class PosEncTest : public ::testing::Test {
 protected:
  void SetUp() override {
  ...
  bool ArraysNearlyEqual(const std::vector<float>& expected, const std::vector<float>& actual,
                         float tol = 1e-5) {
    ...
    for (size_t i = 0; i < expected.size(); ++i) {
      if (std::abs(expected[i] - actual[i]) > tol) {
        std::cout << "Mismatch at index " << i << ": expected " << expected[i] << ", actual "
                  << actual[i] << std::endl;
        ++mismatch_count;
      }
    }
  ...
  void computeRoPEReference(const std::vector<float>& input, std::vector<float>& output,
                            const std::vector<float>& freq, int pos, int rotary_dim) {
  ...
  void ComputeRoPEReference_interleave(const std::vector<float>& input, std::vector<float>& output,
                                       const std::vector<float>& freq, int pos, int rotary_dim,
                                       int bdx) {
    ...
    for (uint32_t thread_idx = 0; thread_idx < bdx; ++thread_idx) {
      for (uint32_t i = 0; i < vec_size; ++i) {
        ...
        if (idx < rotary_dim) {
          ...
          output[idx] = x_i * cos_val + ((i % 2 == 0) ? -x_j : x_j) * sin_val;
        } else {
  ...
  void ComputeRoPEReference_cos_sin_interleave_reuse_half(const std::vector<float>& input,
                                                          const std::vector<float>& cos,
                                                          const std::vector<float>& sin,
                                                          std::vector<float>& output,
                                                          int rotary_dim, int vec_size, int bdx) {
    for (uint32_t thread_idx = 0; thread_idx < bdx; ++thread_idx) {
      for (uint32_t i = 0; i < vec_size; ++i) {
        uint32_t idx = thread_idx * vec_size + i;
        if (idx < rotary_dim) {
...
__global__ void test_kernel_normal(T* d_input, float* d_freq, float* d_output, int32_t pos,
                                   uint32_t rotary_dim) {
  ...
  if (thread_idx < bdx) {
    ...
    result = vec_apply_llama_rope<vec_size, bdx>(d_input, freq, pos, rotary_dim);
...
template <uint32_t vec_size, uint32_t bdx, typename T>
__global__ void test_kernel_interleave(T* d_input, float* d_freq, float* d_output, int32_t pos,
                                       uint32_t rotary_dim) {
  int thread_idx = threadIdx.x;
  if (thread_idx < bdx) {
    vec_t<float, vec_size> freq;
    ...
    vec_t<float, vec_size> result;
    result = vec_apply_llama_rope_interleave<vec_size, bdx>(d_input, freq, pos, rotary_dim);
    result.store(d_output + thread_idx * vec_size);
  }
}
...
template <uint32_t vec_size, uint32_t bdx, typename T>
__global__ void test_kernel_cos_sin_interleave_reuse_half(T* d_input, float* d_cos, float* d_sin,
                                                          float* d_output, uint32_t rotary_dim) {
  int thread_idx = threadIdx.x;
  if (thread_idx < bdx) {
    ...
    vec_t<float, vec_size> result =
        vec_apply_llama_rope_cos_sin_interleave_reuse_half<vec_size, bdx>(d_input, cos_vec, sin_vec,
                                                                          rotary_dim);
    result.store(d_output + thread_idx * vec_size);
  }
}
...
TYPED_TEST(PosEncTest, TestVecApplyLlamaRope) {
  ...
  for (uint32_t i = 0; i < vec_size; ++i) {
    if (i < rotary_dim / 2) {
      h_freq[i] = 1.0f / std::pow(rope_theta, static_cast<float>(2 * i) / rotary_dim);
    } else {
      // For non-interleaved mode
      h_freq[i] =
          1.0f / std::pow(rope_theta, static_cast<float>(2 * (i - rotary_dim / 2)) / rotary_dim);
    }
  }
  ...
  this->ComputeRoPEReference_interleave(h_input_float, h_ref_output_interleave, h_freq, pos,
                                        rotary_dim, bdx);
  this->computeRoPEReference(h_input_float, h_ref_output_normal, h_freq, pos, rotary_dim);
  ...
  ASSERT_EQ(hipMalloc(&d_output_normal, head_dim * sizeof(float)), hipSuccess);
  ASSERT_EQ(hipMalloc(&d_output_interleave, head_dim * sizeof(float)), hipSuccess);

  // Copy data to device
  ASSERT_EQ(hipMemcpy(d_input, h_input.data(), head_dim * sizeof(TypeParam), hipMemcpyHostToDevice),
            hipSuccess);
  ASSERT_EQ(hipMemcpy(d_freq, h_freq.data(), vec_size * sizeof(float), hipMemcpyHostToDevice),
            hipSuccess);

  // Launch kernel
  test_kernel_interleave<vec_size, bdx>
      <<<dim3(1), dim3(bdx)>>>(d_input, d_freq, d_output_interleave, pos, rotary_dim);
  test_kernel_normal<vec_size, bdx>
      <<<dim3(1), dim3(bdx)>>>(d_input, d_freq, d_output_normal, pos, rotary_dim);
...
TYPED_TEST(PosEncTest, TestVecApplyLlamaRopeCosSinInterleaveReuseHalf) {
  ...
  this->ComputeRoPEReference_cos_sin_interleave_reuse_half(
      h_input_float, h_cos, h_sin, h_expected_output, rotary_dim, vec_size, bdx);
  ...
  ASSERT_EQ(hipMemcpy(d_input, h_input.data(), head_dim * sizeof(TypeParam), hipMemcpyHostToDevice),
            hipSuccess);
  ASSERT_EQ(hipMemcpy(d_cos, h_cos.data(), vec_size * sizeof(float), hipMemcpyHostToDevice),
            hipSuccess);
  ASSERT_EQ(hipMemcpy(d_sin, h_sin.data(), vec_size * sizeof(float), hipMemcpyHostToDevice),
            hipSuccess);

  // Launch kernel
  test_kernel_cos_sin_interleave_reuse_half<vec_size, bdx>
      <<<dim3(1), dim3(bdx), 0, 0>>>(d_input, d_cos, d_sin, d_output, rotary_dim);

  // Copy result back
  std::vector<float> h_output(head_dim);
  ASSERT_EQ(hipMemcpy(h_output.data(), d_output, head_dim * sizeof(float), hipMemcpyDeviceToHost),
            hipSuccess);
