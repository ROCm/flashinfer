...
constexpr uint32_t HALF_ELEMS_PER_THREAD = 4;   // Each thread processes 4 half elements
...
void SimulateKReadPattern(std::vector<int>& thread_ids_reading_offsets) {
  ...
  constexpr uint32_t K_SMEM_COLUMN_ADVANCE = 16 / HALF_ELEMS_PER_THREAD;  // = 4 for MI300
  ...
  for (uint32_t tid = 0; tid < WARP_SIZE; tid++) {
    ...
    uint32_t k_smem_offset_r = get_permuted_offset_linear<UPCAST_STRIDE_K>(
        warp_idx_kv * NUM_MMA_KV * 16 + 4 * (lane_idx / 16) + lane_idx % 4, (lane_idx % 16) / 4);
    ...
    for (uint32_t mma_d = 0; mma_d < NUM_MMA_D_QK; ++mma_d) {
      for (uint32_t mma_kv = 0; mma_kv < NUM_MMA_KV; ++mma_kv) {
        ...
        if (tid == 0) {
          std::cout << "Thread " << tid << " k_smem_offset_r " << k_smem_offset_r << '\n';
        }

        // Simulate loading a matrix fragment
        for (uint32_t reg_id = 0; reg_id < INT32_ELEMS_PER_THREAD; reg_id++) {
          if (read_row < grid_height && read_col < grid_width) {
            thread_ids_reading_offsets[read_row * grid_width + read_col] = tid;
          }
        ...
        k_smem_offset_r = advance_offset_by_row_linear<16, UPCAST_STRIDE_K>(k_smem_offset_r);
      }
      ...
      k_smem_offset_r =
          advance_offset_by_column_linear<K_SMEM_COLUMN_ADVANCE>(k_smem_offset_r, mma_d) -
...
void RunKReadPatternTest() {
  ...
  for (int c = 0; c < grid_width; c++) {
    printf("%3d ", c);
    if (c == 15 && grid_width > 16) printf("| ");  // Divider for HEAD_DIM=128
  }
  printf("\n   +");
  for (int c = 0; c < grid_width; c++) {
    printf("----");
  ...
  // Print statistics
  printf("\nStatistics:\n");
  printf("- Positions read: %d/%d (%.1f%%)\n", grid_height * grid_width - unread,
         grid_height * grid_width,
         100.0f * (grid_height * grid_width - unread) / (grid_height * grid_width));
  printf("- Unread positions: %d/%d (%.1f%%)\n", unread, grid_height * grid_width,
...
TEST(MI300KReadPatternTest, HeadDim64_NumMmaKV1) { RunKReadPatternTest<64, 1>(); }
