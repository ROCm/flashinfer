  ...
  for (uint32_t tid = 0; tid < 64; tid++) {
    ...
    for (uint32_t mma_d = 0; mma_d < NUM_MMA_D_QK; ++mma_d) {
      for (uint32_t mma_q = 0; mma_q < NUM_MMA_Q; ++mma_q) {
        ...
        q_smem_offset_r = advance_offset_by_row_linear<16, UPCAST_STRIDE_Q>(q_smem_offset_r);
      }

      // Reset row position and advance to next column, exactly as in
      // compute_qk
      q_smem_offset_r = advance_offset_by_column_linear<4>(q_smem_offset_r, mma_d) -
...
TEST(MI300ReadPatternTest, HeadDim128_NumMmaQ1) { RunReadPatternTest<128, 1>(); }

TEST(MI300ReadPatternTest, HeadDim64_NumMmaQ2) { RunReadPatternTest<64, 2>(); }

TEST(MI300ReadPatternTest, HeadDim128_NumMmaQ2) { RunReadPatternTest<128, 2>(); }
