...
template <uint32_t HEAD_DIM> struct TestKernelTraits
{
...
__global__ void test_init_rope_freq_kernel(float *output_freq,
        ...
        for (uint32_t mma_d = 0; mma_d < KTraits::NUM_MMA_D_VO / 2; ++mma_d) {
            for (uint32_t j = 0; j < 4; ++j) {
                ...
                uint32_t feature_idx =
                    flashinfer::get_feature_index<HEAD_DIM>(mma_d, lane_idx, j);
...
protected:
    ...
    std::vector<float> apply_cpu_rope(size_t offset,
                                      float rope_scale = 1.0f,
                                      float rope_theta = 10000.0f)
    {
        return cpu_reference::apply_llama_rope(q.data(), head_dim, offset,
                                                ...
                                                float rope_theta = 10000.0f)
    {
        ...
        for (size_t k = 0; k < head_dim; ++k) {
            ...
            float frequency =
                (1.0f / rope_scale) / std::pow(rope_theta, freq_base);
                                                ...
                                                float rope_theta = 10000.0f)
    {
        ...
        if (head_dim == 64) {
            test_init_rope_freq_kernel<64><<<grid, block>>>(
                d_output_freq, rope_rcp_scale, rope_rcp_theta);
        }
        ...
        FI_GPU_CALL(hipMemcpy(gpu_frequencies.data(), d_output_freq,
                              output_size, hipMemcpyDeviceToHost));
    ...
    apply_cpu_rope_all_sequences(size_t kv_len = 1000,
                                 float rope_scale = 1.0f,
                                 float rope_theta = 10000.0f)
    {
        ...
        DISPATCH_head_dim(head_dim, HEAD_DIM, {
            ...
            tensor_info_t info(qo_len, kv_len, num_qo_heads, num_qo_heads,
                               QKVLayout::kHND, HEAD_DIM);
                 ...
                 ++qo_head_idx)
            {
                for (size_t q_idx = 0; q_idx < qo_len; ++q_idx) {
                    ...
                    auto q_rotary_local = cpu_reference::apply_llama_rope_debug(
                        ...
                        head_dim, offset, rope_scale, rope_theta);
...
TEST_P(LLamaRopeTestWithFP16, TestInitRopeFreq)
{
    ...
    for (size_t i = 0; i < std::min(16u, this->head_dim); ++i) {
        ...
        std::cout << i << "\t" << cpu_frequencies[i] << "\t\t"
                  << gpu_frequencies[i] << "\t\t" << diff << std::endl;
    }
...
INSTANTIATE_TEST_SUITE_P(
    LLamaRopeTestWithFP16,
    LLamaRopeTestWithFP16,
    ::testing::Values(
        std::make_tuple(256, 1, 64) // qo_len=256, num_qo_heads=1, head_dim=64
        ));
