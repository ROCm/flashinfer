...
at::Tensor
BatchPrefillWithKVCachePlan(at::Tensor float_workspace_buffer,
                            ...
                            bool causal)
{
    ...
    hipError_t status = PrefillPlan<IdType>(
        ...
        kv_indptr.data_ptr<IdType>(), total_num_rows, batch_size, num_qo_heads,
        num_kv_heads, head_dim_qk, head_dim_vo, page_size, enable_cuda_graph,
        /*sizeof_dtype_o=*/2, stream);

    TORCH_CHECK(status == hipSuccess, "Failed to plan prefill with error: ",
                                          ...
                                          ADDITIONAL_FUNC_PARAMS)
{
    ...
    int64_t num_kv_heads =
        (kv_layout == QKVLayout::kNHD) ? k.size(1) : k.size(0);
    uint32_t q_stride_n = q.stride(0), q_stride_h = q.stride(1), k_stride_n,
        ...
        [&] {
            ...
            if (plan_info.split_kv) {
                ...
                tmp_v = GetPtrFromBaseOffset<DTypeO>(float_buffer_ptr,
                                                     plan_info.v_offset);
                tmp_s = GetPtrFromBaseOffset<float>(float_buffer_ptr,
                                                    plan_info.s_offset);
            ...
            DISPATCH_CTA_TILE_Q(plan_info.cta_tile_q, CTA_TILE_Q, {
                status = flashinfer::BatchPrefillWithRaggedKVCacheDispatched<
                    ...
                    AttentionVariant, RaggedParams>(params, tmp_v, tmp_s,
                                                    stream);
            });
        ...
        [&] {
            ...
            DISPATCH_CTA_TILE_Q(plan_info.cta_tile_q, CTA_TILE_Q, {
                status = flashinfer::BatchPrefillWithPagedKVCacheDispatched<
                    CTA_TILE_Q, HEAD_DIM_QK, HEAD_DIM_VO, POS_ENCODING_MODE,
                    /*use_fp16_qk_reduction=*/USE_FP16_QK_REDUCTION, MASK_MODE,
                    AttentionVariant, PagedParams>(params, tmp_v, tmp_s,
                                                   stream);
            });
