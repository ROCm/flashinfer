...
# Pin this worker to a dedicated supported GPU BEFORE torch initializes the ROCm/CUDA
# runtime.  PYTEST_XDIST_WORKER is injected into each worker subprocess by
# pytest-xdist before any Python code runs ("gw0", "gw1", ...).  We map the worker
# index through get_supported_device_indices() so that workers are pinned only to
# FlashInfer-supported GPUs and unsupported integrated GPUs are never assigned.
# Setting HIP_VISIBLE_DEVICES here makes the ROCm runtime start up with exactly one
...
if _xdist_worker.startswith("gw"):
    ...
    _supported_devices = get_supported_device_indices()
    if _worker_idx >= len(_supported_devices):
        raise RuntimeError(
            f"xdist worker {_xdist_worker} has no corresponding supported GPU "
            f"(only {len(_supported_devices)} supported device(s) found)."
        )
    _gpu_index = _supported_devices[_worker_idx]
