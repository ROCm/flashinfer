...
**Soft-capped causal prefill falls back to `fa2` on gfx950.** AITER's
`mha_varlen_fwd` miscomputes `logits_soft_cap` at `head_dim=128` there, so
`auto` declines it and `backend="aiter"` raises rather than returning wrong
numbers. gfx942 is unaffected and uses AITER as normal — see
